...
Anthropic's recent article on [building agents that reach production systems with MCP](https://claude.com/blog/building-agents-that-reach-production-systems-with-mcp) arrives at the same conclusion: mature integrations should ship all three layers — an API foundation, a CLI for local and CI/CD work, and an MCP server for cloud-hosted agents. Their key observation is that as agents move from local development into cloud deployments (think Claude's Managed Agents or similar hosted runtimes), MCP becomes "the critical layer" because a CLI alone can't reach environments without shell access. That's a fair point, and it's exactly why GA CLI is designed so that its schema introspection (`--describe`) can serve as the bridge to auto-generating MCP tool definitions — you build the CLI first, and the MCP layer follows naturally without duplicating effort.
...
This is where **skills** come in. Skills are one of the most powerful ways to customize AI agents (e.g., in Claude Code) for specific, repeatable workflows. Think of a skill as a structured, reusable playbook that tells the agent not just *what* tools it has, but *how* to use them in context, the right order, the right conventions, the gotchas to watch out for. Anthropic's [production agents article](https://claude.com/blog/building-agents-that-reach-production-systems-with-mcp) calls this out explicitly: skills provide the **procedural knowledge** while tools (CLI, MCP, APIs) provide the **capabilities**. The two are complementary, and the most reliable agent workflows combine both.
