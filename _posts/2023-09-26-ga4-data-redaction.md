...
3. Processing data on GA4's servers, where data from users' browsers is gathered and relayed to the GA4 endpoint.
...
Drawing a parallel to Universal Analytics (UA), certain platform configurations dictate data processing before its presentation to the analyst. Examples include the duration for which data is kept (Data Retention settings) or how specific traffic types are treated (like exclusion from datasets or marking via a dimension). This mirrors UA's Property and View settings that influenced data processing.
...
You have the flexibility to opt for redacting either email addresses or URL query parameters. If you select the latter, you can detail which parameters you want redacted. For example, if you wish to redact the firstname and lastname parameters, simply list them in the provided text box, using commas as separators.
...
Let's have a closer look at what Google means by "best-effort basis". As shown earlier, we can expect this logic to present in gtag.js and when searching my specific instance of the code, we indeed find the following part of the code that holds the specifications from my web data stream:
...
- `[A-Z0-9._-]|%25|%2B)+%40[A-Z0-9.-]` (case insensitive, URL-encoded)
...
When we look at the privacy controls in GA4, we can see that they are spread across different areas of the platform and the data flow. The following diagram provides an overview of the different privacy controls in GA4 and where in the data flow they are located:
...
- Pre-Data Collection: This is where you prevent data from being picked up client-side altogether. Here you can configure the Data Redaction feature or apply your own redaction logic using custom variables in the GTM web container.
